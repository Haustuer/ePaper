...
void mercator_projection(double lat, double lon,   int *x, int *y) {
    ...
    Debug(" min: %f, max:%f  , lat: %f, norm:%f  \r\n ",Mminlat,Mmaxlat,Mlat,norm);
...
int main(int argc, char *argv[])
{
    ...
    case 3:
        ...
        case 4:
                ...
                Display_Icon(Init_Target_Memory_Addr,680,1180,0);
        break;
        ...
        for (int x=0;x<1400;x+=50){
            for (int y=0;y<1200;y+=50){
                Debug("x:%d y:%d \n",x,y);
                Display_BMP_Patch(Init_Target_Memory_Addr, x, y, 200, 200);
